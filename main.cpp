// Based on Dear ImGui: standalone example application for DirectX 12
...
// trying to flush swapchain on resize to remove resize artifacts because DWM is out of sync with presentation
// does not work :(
//#define FORCE_RESIZE_REDRAW
...
void Render(bool force = false)
{
  ...
  if (force)
  {
    ...
    assert(SUCCEEDED(g_pSwapChain->Present(0, DXGI_PRESENT_RESTART)));
  }
  ...
  DXGI_PRESENT_PARAMETERS rec{ .DirtyRectsCount = 0, .pScrollRect = NULL };
  auto flags = force ? DXGI_PRESENT_DO_NOT_SEQUENCE : 0;
  g_pSwapChain->Present1(1, flags, &rec); // Present with vsync
  //g_pSwapChain->Present1(0, 0, &rec); // Present without vsync
...
bool CreateDeviceD3D(HWND hWnd)
{
  ...
  DXGI_SWAP_CHAIN_DESC1 sd;
  {
    ...
    //sd.Scaling = DXGI_SCALING_STRETCH; // if use composition swapchain
    sd.Stereo = FALSE;
  }
...

  {
    ...
    if (adapter->EnumOutputs(0, &adapterOutput) != S_OK)
    {
      return false;
    }

  {
    // if the adapter reports that it does not support multi-plane overlays, we cannot achieve Hardware Independent Flip
    // enjoy the awful DWM latency :(
    IDXGIOutput6* output;
    if (adapterOutput->QueryInterface(IID_PPV_ARGS(&output)) != S_OK)
      return false;
    adapterOutput->Release();
    adapter->Release();
    std::cout << ((output->SupportsOverlays() == FALSE) ? "DOESN'T SupportsOverlays" : "SupportsOverlays") << std::endl;
    DXGI_HARDWARE_COMPOSITION_SUPPORT_FLAGS pls;
    if (output->CheckHardwareCompositionSupport((UINT*)&pls) != S_OK)
      return false;
    output->Release();
  }
